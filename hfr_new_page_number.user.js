...
function showconfig() {
  ...
  npn_cb_openatend.checked = open_at_end;
  if(gm4) {
    npn_cb_openatend.checked = false;
    npn_cb_openatend.disabled = true;
    npn_lb_openatend.style.color = "#808080";
  }
...
function drapal_mouseup(e) {
  ...
  if(mass_opener && (max_tab > 1) && (this.hasAttribute("data-npn-multi-page-nb")) && (e.button === 1) &&
    ...
    url_array.forEach(async function(url, index, array) {
      await GM.openInTab(url, open_in_background);
      //await console.log(url, index, array.length);
