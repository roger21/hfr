...
// 3.0.5 (19/03/2020) :
// - correction d'un bug sur la gestion de la fausse détection du smiley ":/" dans les urls
// 3.0.4 (12/03/2020) :
...
// 3.0.2 (12/03/2020) :
// - correction de l'affichage du panneau des favoris en réponse rapide pour éviter le décalage de la page ->
// au chargement (problème signalé par Lt Ripley)
// 3.0.1 (07/03/2020) :
...
style.textContent =
  ...
  */
  ...
  "div#gm_hfr_vsf_r21_keywords_tooltip.gm_hfr_vsf_r21_locked{background:linear-gradient(#ffffff, #fff0f0);}" +
  ...
  "z-index:1006;}" +
  "div#gm_hfr_vsf_r21_keywords_popup.gm_hfr_vsf_r21_locked{background:linear-gradient(#ffffff, #fff0f0);}" +
  ...
  "textarea.gm_hfr_vsf_r21_keywords_textarea{margin:0;padding:1px 4px;border:1px solid #c0c0c0;" +
  ...
  "textarea.gm_hfr_vsf_r21_keywords_textarea:disabled{background:#f0f0f0;color:#707070;}" +
...
function show_tooltip(p_event) {
  ...
  keywords_tooltip_timer = window.setTimeout(function() {
    access_keywords(get_keywords, function(p_smiley_img, p_keywords, p_locked) {
      ...
      keywords_tooltip.classList.remove("gm_hfr_vsf_r21_locked");
      if(p_locked) {
      ...
      let l_tooltip_width = keywords_tooltip.offsetWidth;
...
function show_popup(p_event) {
  ...
  if(hash_check !== "") {
    ...
    keywords_popup.classList.remove("gm_hfr_vsf_r21_locked");
    ...
    access_keywords(get_keywords, function(p_smiley_img, p_keywords, p_locked) {
      ...
      if(p_locked) {
        keywords_popup.classList.add("gm_hfr_vsf_r21_locked");
...
function import_backup(p_event) {
  let l_backup;
  try {
    l_backup = JSON.parse(this.result);
  } catch (e) {
...
// function de gestion de la lecture du fichier de sauvegarde lors de la restauration
function read_backup(p_event) {
  let l_file = this.files[0];
  let l_reader = new FileReader();
  l_reader.addEventListener("loadend", import_backup, false);
  l_reader.readAsText(l_file);
}
...
function get_smiley_list(p_message) {
  ...
  if(p_message !== "") {
    ...
    p_message = p_message.replace(/https?:\//gi, " ");
