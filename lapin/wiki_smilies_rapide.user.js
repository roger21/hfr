...
if ($('content_form'))
{
  ...
  var proceedShortcut = function (event, textAreaId)
  {
    ...
    {
      event.preventDefault();
      var url = 'https://generateurs.super-h.fr/bulle/?t=';
    ...
    img.parentNode.addEventListener('click', function()
    {
      ...
      var timer = setInterval(function()
      {
        if ($('rep_editin_' + numreponse))
        {
          ...
          // Wiki Smilies
          $('search_smilies_edit_' + numreponse).addEventListener('keyup', function() { findSmilies('search_smilies_edit_' + numreponse, 'dynamic_smilies_edit_' + numreponse); }, false);
          // Raccourcis
  ...
  /* Ajout de l'evenement sur le champ de recherche des smilies */

  $('search_smilies').addEventListener('keyup', function() { findSmilies('search_smilies', 'dynamic_smilies'); }, false);
...
({
  check4Update : function()
  {
    ...
    var currentVersion = GM_getValue('currentVersion', '0.6.3a');
    // On met éventuellement la version stockée à jour avec la version courante, si la version courante est plus récente
    if (autoUpdate.isLater('0.6.3a', currentVersion))
    {
      GM_setValue('currentVersion', '0.6.3a');
      currentVersion = '0.6.3a';
    }
    ...
    else if (autoUpdate.isLater(currentVersion, '0.6.3a'))
    {
      ...
      {
        GM_openInTab(mirrorUrl + 'wiki_smilies_rapide.user.js');
      }
      );
    }
