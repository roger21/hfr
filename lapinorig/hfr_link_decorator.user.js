...
({
	check4Update : function()
	{
		...
		var currentVersion = GM_getValue('currentVersion', '0.0.3');
		// On met éventuellement la version stockée à jour avec la version courante, si la version courante est plus récente
		if (autoUpdate.isLater('0.0.3', currentVersion))
		{
			GM_setValue('currentVersion', '0.0.3');
		...
		else if (autoUpdate.isLater(currentVersion, '0.0.3'))
		{
			GM_registerMenuCommand("[HFR] Liens explicites -> Installer la version " + currentVersion, function()
			{
				GM_openInTab(mirrorUrl + 'others/hfr_link_decorator.user.js');
			}
			);
		}
		...
		{
			var checkUrl = mirrorUrl + 'getLastVersion.php5?name=' + encodeURIComponent('[HFR] Liens explicites');
			...
			GM_xmlhttpRequest({
				...
				onload: function(response)
				{
					...
					{
						if (confirm('Une nouvelle version de [HFR] Liens explicites est disponible : ' + lastVersion + '\nVoulez-vous l\'installer ?'))
						{
							GM_openInTab(mirrorUrl + 'others/hfr_link_decorator.user.js');
						}
						else
