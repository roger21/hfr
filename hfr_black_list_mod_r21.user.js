...
// historique :
// 3.5.8 (25/04/2020) :
// - gestion des pseudos mal tronqués par le forum
// 3.5.7 (12/03/2020) :
...
Promise.all([
  ...
  if(root) {
    ...
    function hideQuotes() {
      ...
      for(let quote of quotes) {
        ...
        if(title && title.textContent.includes(" a écrit :")) {
          ...
          if(isQuotedPseudoBlacklisted(pseudo)) {
    ...
    // vérifie si un pseudo quoté est dans la black liste
    function isQuotedPseudoBlacklisted(pseudoAVerifier) {
      if(pseudoAVerifier.length > 10) {
        let encode = encoder.encode(pseudoAVerifier);
        let temp = "";
        let cpt = 0;
        while(cpt < encode.length) {
          temp += encode.slice(cpt, cpt + 10).join();
          if(cpt + 10 < encode.length) {
            temp += zero;
          }
          cpt += 10;
        }
        temp = Uint8Array.from(temp.split(",").map(e => parseInt(e)));
        pseudoAVerifier = decoder.decode(temp);
      }
      return blacklist.indexOf(getNormalPseudo(pseudoAVerifier)) >= 0;
    }
